"""Agent Cognitive Workload Engine — cognitive effort budget across load types.

Models the finite cognitive effort budget using Cognitive Load Theory: intrinsic
load (task difficulty), extraneous load (presentation overhead), and germane
load (schema-building effort). A snapshot aggregates current loads, compares
the total against a capacity envelope, and classifies the agent from underloaded
through optimal to saturated. When overloaded, allocation decisions redistribute
resources via shedding, deferring, delegating, chunking, sequencing, or offloading.
